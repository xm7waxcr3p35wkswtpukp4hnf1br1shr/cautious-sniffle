...
const C = {
  ...
  t0: "#f0f0f2", t2: "rgba(240,240,242,0.35)", t3: "rgba(240,240,242,0.18)",
  ton: "#0098ea", tonDim: "rgba(0,152,234,0.15)",
  green: "#35c96b", red: "#f04040",
} as const;
...
export default function LoginClient() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.ok) { setError(data.error ?? "Authentication failed"); setLoading(false); return; }
      setSuccess(true);
  ...
  const disabled = loading || !apiKey.trim() || success;
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        ::-webkit-scrollbar { width: 3px; } ::-webkit-scrollbar-thumb { background: ${C.lineHi}; }
      ...
      <div style={{
        ...
        padding: "24px", fontFamily: "var(--font-mono)", position: "relative", overflow: "hidden",
      }}>
        {/* Grid bg */}
        <div style={{
        ...
        <div style={{
          ...
          animation: mounted ? "fadeUp 0.25s ease forwards" : "none", opacity: mounted ? 1 : 0,
        }}>
          {/* Header */}
          <div style={{ marginBottom: "28px", textAlign: "center" }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: "16px" }}>
              <div style={{ width: 44, height: 44, borderRadius: "4px", background: "#161618", border: `0.5px solid ${C.lineHi}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <TonLogo size={22} />
              </div>
            </div>
            <h1 style={{ fontSize: "18px", fontWeight: 700, color: C.t0, margin: "0 0 6px", letterSpacing: "-0.01em", ...F }}>Username Tool</h1>
            <p style={{ fontSize: "11px", color: C.t2, margin: 0, letterSpacing: "0.03em", ...F }}>Enter your API key to continue</p>
          </div>
          ...
          <div style={{ background: C.bg1, border: `0.5px solid ${C.line}`, borderRadius: "4px", overflow: "hidden" }}>
            ...
            <div style={{ background: C.bg2, borderBottom: `0.5px solid ${C.line}`, padding: "8px 14px", display: "flex", alignItems: "center", gap: "6px" }}>
              ...
              <span style={{ fontSize: "10px", color: C.t3, marginLeft: "6px", letterSpacing: "0.06em", ...F }}>Auth · API key</span>
            </div>

            <div style={{ padding: "20px 18px 18px" }}>
              {error && (
                <div style={{ padding: "8px 11px", background: "rgba(240,64,64,0.07)", border: "0.5px solid rgba(240,64,64,0.28)", borderRadius: "2px", color: C.red, fontSize: "11px", marginBottom: "14px", display: "flex", gap: "7px", alignItems: "flex-start", animation: "fadeUp 0.15s ease forwards", ...F }}>
                  <span style={{ flexShrink: 0 }}>✕</span><span>{error}</span>
                </div>
              )}
              {success && (
                <div style={{ padding: "8px 11px", background: "rgba(53,201,107,0.07)", border: "0.5px solid rgba(53,201,107,0.28)", borderRadius: "2px", color: C.green, fontSize: "11px", marginBottom: "14px", display: "flex", gap: "7px", alignItems: "center", animation: "fadeUp 0.15s ease forwards", ...F }}>
                  <span>✓</span><span>Authenticated · redirecting…</span>
                </div>
              )}

              <label style={{ display: "block", fontSize: "10px", color: C.t2, letterSpacing: "0.08em", marginBottom: "6px", textTransform: "uppercase", ...F }}>
                API Key
              </label>
              ...
              <div style={{
                ...
                borderRadius: "2px", overflow: "hidden", transition: "border-color 120ms ease", marginBottom: "12px",
              }}>
                ...
                <input
                  ...
                  style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: C.t0, fontSize: "13px", padding: "10px 10px", ...F, letterSpacing: showKey ? "normal" : "0.05em" }}
                />
                <button onClick={() => setShowKey(v => !v)} style={{
                  background: "transparent", border: "none", borderLeft: `0.5px solid ${C.line}`,
                  padding: "0 12px", height: "100%", minHeight: "40px", color: C.t2, cursor: "pointer",
                  display: "flex", alignItems: "center", transition: "color 100ms ease", flexShrink: 0,
                }}
              ...
              <button onClick={() => void handleLogin()} disabled={disabled} style={{
                width: "100%", padding: "11px",
                background: disabled ? "rgba(240,240,242,0.06)" : C.t0,
                color: disabled ? C.t3 : C.bg0,
                border: "none", borderRadius: "2px",
                fontSize: "12px", fontWeight: 700, letterSpacing: "0.06em",
                cursor: disabled ? "not-allowed" : "pointer",
                display: "flex", alignItems: "center", justifyContent: "center", gap: "8px",
                transition: "background 120ms ease, color 120ms ease", ...F,
              }}
                onMouseEnter={e => { if (!disabled) (e.currentTarget as HTMLButtonElement).style.background = "rgba(240,240,242,0.87)"; }}
                onMouseLeave={e => { if (!disabled) (e.currentTarget as HTMLButtonElement).style.background = C.t0; }}
              >
                {loading ? <><Spinner />Verifying…</> : success ? "✓ Authenticated" : "Sign in"}
              </button>
            </div>

            <div style={{ padding: "8px 18px 10px", borderTop: `0.5px solid ${C.line}`, background: C.bg2 }}>
              <p style={{ fontSize: "10px", color: C.t3, margin: 0, ...F }}>Contact Admin for Access</p>
            </div>
          </div>

          <p style={{ textAlign: "center", fontSize: "10px", color: C.t3, marginTop: "18px", ...F }}>
